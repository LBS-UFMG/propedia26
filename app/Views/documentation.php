...
The database is equipped with a user-friendly web interface and powerful search tools, allowing users to query complexes by PDB ID, peptide sequence, protein sequence, specific interaction motifs, or thermodynamic parameters. Furthermore, Propedia integrates advanced analytical capabilities, such as multiple sequence alignment and clustering based on peptide similarity, enabling comparative studies and the identification of binding patterns.<br>
Key Highlights:<br>
Curated Dataset: A comprehensive collection of protein-peptide complexes from the PDB, carefully validated and annotated.<br>
Dual Search Modes: Supports both text-based queries (e.g., PDB ID, UniProt ID) and sequence-based similarity searches (BLAST).
Advanced Filtering: Enables refinement of results by experimental method, resolution, interaction energy, and more.<br>
Integrated Analysis Tools: Built-in tools for visualizing interfaces, aligning sequences, and clustering complexes.<br>
Open Access: All data is freely available for download, supporting reproducible research.<br>
<h3> What's new in version 26</h3>
Propedia 26 introduces major updates that significantly expand the database and enhance its analytical power.
<h3> Expanded dataset</h3>
Increased complex count: The updated version of Propedia now includes 78,148 protein-peptide complexes, representing nearly a fourfold increase in data coverage compared to the previous release (19,813 complexes), an increase of approximately 3.9-fold, as shown in Figure 1.<br>
...
Modernized layout: Complete visual overhaul with improved navigation and responsive design (Figure 2).<br>
Enhanced search page: More intuitive organization of search options and filters.<br>
...
Peptide clustering: Implementation of a novel peptide similarity clustering algorithm that groups complexes based on peptide sequence similarity, enabling evolutionary and functional analysis (Figure 3), more details following.
...
In version 26, the complex details page has been extensively redesigned to offer a much deeper interaction analysis: it now displays atomic data with precise distance measurements and clear categorization of interaction types (hydrogen bonds, hydrophobic contacts, etc.). In addition, complete structural metrics, such as interface area and interaction energy, which were previously absent or very basic, have been incorporated. The presentation of the data has also been reorganized: in v26, the information is distributed across tabs (structure, energy, sequence) for greater clarity; in the old version, everything was on a single page with less organization. From a computational standpoint, energy calculations have been improved with updated algorithms (e.g., NACCESS) with more refined parameterization, while the previous version applied basic calculations with limited validation. These topics are shown in Table 1 and they will be discussed in more detail in section X.<br>
Table 1. News in Propedia's property


<h3 class="pt-4 pb-1"> How to cite and license</h3>
To cite PROPEDIA, we recommend referencing both the original article and the most recent publication in the database. If specific features or previous versions are used, the respective publications may also be cited. The original 2021 article presents the first description of the database:<br><br>
Martins, P.M., Santos, L.H., Mariano, D. et al. Propedia: a database for protein–peptide identification based on a hybrid clustering algorithm. BMC Bioinformatics 22, 1 (2021). doi: 10.1186/s12859-020-03881-z.<br><br>
Version 2.3, published in 2023, introduces a new representation approach based on structural signatures:<br><br>
Martins P, Mariano D, Carvalho FC, Bastos LL, Moraes L, Paixão V, and Cardoso de Melo-Minardi R (2023). Propedia v2.3: A novel representation approach for the peptide-protein interaction database using graph-based structural signatures. Front. Bioinform. 3:1103103. doi: 10.3389/fbinf.2023.1103103.<br><br>
...
Upon accessing the home page, users will find an intuitive navigation panel that allows them to quickly explore the main features of the database, including complex search, structural visualization, interaction analysis, and access to download tools. The initial interface features a top navigation bar that directs users to the Home, About, Browse, Clusters, Downloads, and Help pages. In addition, there is a quick search field that allows users to directly search for PDB IDs, peptides, or proteins. The page also includes a highlights panel with information about new features and updates incorporated into version 26. These details are shown in Figure 5.<br>
...
·   	task blastp-short<br>
The task blastp-short parameter activates an optimized version of BLASTP specifically configured to handle short protein sequences, typically with fewer than 30 amino acids (Table C3: [blastp application options. The blastp...].”, 2021). This mode automatically adjusts various internal aspects of the algorithm to maximize sensitivity and detection of real similarity, even when the amount of information (sequence length) is very low.<br>
In implementing the sequence search system in PROPEDIA, -word_size 2 was used in conjunction with -task blastp-short. This choice is directly aligned with the expected behavior for searches involving short peptides, whose sequences have few positions for forming larger “words.”<br>
